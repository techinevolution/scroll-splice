...
Katherine marked that superseding checkpoint **PASS WITH NOTES** on July 14 after confirming live coordinates, live minimap synchronization, all eight Background-region handles, magnet-off/Alt/Option snap override, and Option-drag. The minimap's visual aspect distortion is logged as later polish rather than a blocker. She then approved an optional creator-workflow slice for bounded undo/redo, one explicit local-browser save slot, reload/reopen, a blank new episode, and only the requested **File** and **Edit** menus. That local implementation is complete and validated without changing the format-v3 episode document: 154 unit tests, strict typecheck, ESLint, production build, two Chromium stories, supported-size visual inspection, and the [local save and history screenshot](docs/progress/2026-07-14-local-save-and-history.png) pass. Katherine's hands-on review of this new slice and any push remain pending. Remote `main` remains at `6d6437e`, which includes the earlier implementation published through `8a493a2`. The existing [stable editing, guides, and resize screenshot](docs/progress/2026-07-14-stable-editing-guides-and-resize.png) records the preceding fixed-width checkpoint and remains labeled as historical evidence.
...
Import, persistence, undo, a broader transform system, creator-controlled reordering, production export, accounts, OAuth, OpenAI model access, autonomous generation, and direct publishing are deliberately outside the **required** milestone. The local corrective work and the newly approved local history/save/menu slice are optional creator-facing additions; they do not enlarge what ScrollSplice must demonstrate to qualify as a complete contest MVP. Rotation, flipping, crop, perspective, and freeform distortion remain deferred. If the human editor and every submission dependency are already stable, a narrowly bounded OpenAI image-generation proof may be attempted as stretch work; the submitted editor must not depend on it.

### Creator-ready MVP — phased beyond the core

The full creator-ready milestone adds local asset import, saving and reopening, safe undo, creator-controlled reordering, reader preview, and a validated export pipeline. The current optional slice implements only the first local foundation for saving/reopening and undo/redo; import, reusable binary assets, reordering, preview, recovery, and production export remain later work. This creator-ready progress does not make the full milestone a submission requirement. WEBTOON requirements are discovery inputs for the future exporter, not a reason to enlarge the one-week minimum.
...
Katherine's July 14 A/B/C human-test checkpoint and corrective checkpoint D are complete; D passed with the minimap aspect-ratio issue recorded as polish. The newly approved optional slice adds one versioned local-browser save slot, reload/reopen, bounded document history, a blank new episode, and minimal in-app File/Edit menus. Its implementation, validation, and screenshot are complete locally; Katherine's hands-on review and the separate push gate remain. The previously proposed element-opacity and basic Background-fade checkpoint has not started and is not automatically next. Production file export remains a separate later checkpoint because it writes and validates real output files. Real import, autosave/recovery, a project library, tab reordering, deployment, and AI work remain deferred. See [Project Outline](PROJECT_OUTLINE.md#creator-ready-mvp-components) and [Plan](PLAN.md#current-approved-optional-slice-local-history-save-and-minimal-menus).
...
Open the local URL printed by Vite in a desktop Chrome-class browser. The current local build passes 154 unit tests, strict typecheck, ESLint, production build, the complete editor walkthrough, and a focused Chromium save/reload/reopen/New Episode story. Its running UI was visually inspected at 1440 × 900, 1280 × 720, and 1024 × 768; the preceding proportional-handle checkpoint was also checked at 50%, 100%, and 200% zoom.

**Local-save boundary:** Save uses one slot tied to the current browser profile and site origin. Clearing site data, changing browsers/profiles, or using a different origin loses access to that slot. It is not autosave, crash recovery, or a portable project file.
...
16. Choose **File > Save** or press `Mod+S`, reload the page, and confirm the last explicitly saved format-v3 episode opens automatically. Make another unsaved edit, choose **File > Reopen**, accept the discard confirmation, and confirm the last save returns with its history cleared.
...
The current local build passes 154 unit tests, strict typecheck, ESLint, the production build, and two Playwright Chromium stories. The focused story proves an explicit save, automatic restoration after reload, later unsaved edits, confirmed Reopen, blank New Episode, and recovery of the retained save. It was visually inspected at 1440 × 900, 1280 × 720, and 1024 × 768. The existing build warning about a JavaScript chunk slightly above 500 kB is non-blocking and comes from the intentionally bundled React/Konva editor stack; code-splitting is not useful for this single-screen MVP.
