...
The next step is Katherine's **HUMAN TEST and direction checkpoint**, not an automatic move into opacity. Opacity, gradients, tab drag reordering, the full category-based **Add** rail, real import, persistence, undo, export, deployment, and AI work remain deferred until she reviews this interaction set and chooses the next slice. See [Project Outline](PROJECT_OUTLINE.md#creator-ready-mvp-components) and [Plan](PLAN.md#completed-local-multi-slice-work-request-post-review-creator-controls).
...
5. Click the episode title text itself. Confirm the input and cursor appear only after activation, Enter or leaving the field saves, Escape cancels, and an empty edit restores the existing title.
...
9. In an ordinary Background plane, add a solid color region with a chosen start, length, and color. Confirm it spans the episode width and can be selected, moved vertically, hidden, and deleted without changing the pinned base.
