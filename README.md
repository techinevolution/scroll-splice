...
Katherine tested that complete build on July 14. She confirmed that placed-element deletion, the bottom **Add asset** action, expanded-height minimap behavior, and ordinary canvas movement work. Her review also found two interaction defects: activating title editing shifts the fixed **EPISODE** label, and full-width Background color regions feel horizontally janky even though they return to full-width placement. She requested default-on alignment help, profile-derived dotted 1,280-unit WEBTOON candidate boundaries, and later deterministic self-slicing. She explicitly authorized publishing the already passing implementation and this documentation checkpoint to `main`; no code from the separately approved next goal begins in this documentation-only checkpoint.
...
The numbered-plane and editable-backdrop foundation, **Episode Setup and Expandable Scroll**, and the three local post-review checkpoints are now implemented. Empty-plane deletion protects Background plane 1, populated planes, and the final plane in each group; title edits enforce the observed 60-character WEBTOON limit; height changes leave existing content intact while the base and minimap follow the episode. The paperclip now opens the Asset Library and can place only the original code-defined demo swatches. It does not import, persist, or upload a real image.

Katherine's July 14 human-test checkpoint is complete. The next approved implementation goal is **Export-aware polish and alpha controls**: keep the title editor on one stable anchor, remove horizontal jitter from full-width Background color regions, add a default-on magnet control and editor-only candidate slice guides, then add the previously proposed element-opacity and basic Background-fade controls. Production file export remains a separate later checkpoint because it writes and validates real output files. Real import, persistence, undo, tab reordering, deployment, and AI work remain deferred. See [Project Outline](PROJECT_OUTLINE.md#creator-ready-mvp-components) and [Plan](PLAN.md#approved-next-goal-export-aware-polish-and-alpha-controls).
...
Open the local URL printed by Vite in a desktop Chrome-class browser. The current local post-review build has been visually inspected at 1440 × 900, 1280 × 720, and 1024 × 768 and verified through an isolated Playwright Chromium walkthrough.
...
5. Click the episode title text itself. Confirm the input and cursor appear only after activation, Enter or leaving the field saves, Escape cancels, and an empty edit restores the existing title. The July 14 review found that the current input shifts the fixed **EPISODE** label; that layout defect is documented for the next implementation goal.
...
9. In an ordinary Background plane, add a solid color region with a chosen start, length, and color. Confirm it spans the episode width and can be selected, moved vertically, hidden, and deleted without changing the pinned base. The next goal removes its current horizontal drag jitter and makes the full-width center constraint explicit.
...
11. Select an element row from another beat and confirm the canvas reveals it; select an element on the canvas and confirm its group, plane, and row synchronize. Use the trash beside an element eye and confirm only that placed instance is removed.
12. Confirm the active plane's rows follow the story from top to bottom, then drag a selected element and choose **Reset demo** to restore the fixture title, height, visibility, planes, base color, selection, and transient viewport/zoom state.
...
The Build Week MVP does not claim production-ready WEBTOON export. A planned editor-only overlay will show default-on, toggleable gray dotted candidate boundaries derived from the selected export profile—every 1,280 logical units for the currently observed 800 × 1,280 WEBTOON mapping. Those guides are not document elements, do not appear in the minimap or output, and do not themselves create files.
