...
Katherine's follow-up test confirmed that plane deletion works and that the minimap remains accurate after expanding the episode. A second public-safe [post-review baseline screenshot](docs/progress/2026-07-13-post-review-expandable-scroll.png) records the populated-plane view before the next changes. She authorized three separately testable follow-up checkpoints in one work request: direct title/base/element-row controls, safe drag-based height fine-tuning with solid movable Background color regions, and Canvas Zoom with a two-dimensional viewport. None is described as complete until its own validation passes, and this implementation approval does not imply permission to push `main`.
...
The current authorized sequence is: **Direct Creator Controls**, **Safe Precise Height and Background Color Regions**, then **Canvas Zoom and 2D Viewport**. The first removes the permanent title pencil, keeps Base color editable from Layers and the canvas, adds placed-element trash actions, and leaves the disabled paperclip reachable in populated planes. The second retains coarse 1,280-unit extension while adding safe drag grow/shrink and solid movable color regions on ordinary Background planes. The third adds **Fit Width**, a 50–200% range, horizontal access, and an accurate two-dimensional minimap viewport. Each remains pending until its own validation passes. Opacity, gradients, tab drag reordering, the category-based **Add** rail, real import, persistence, undo, export, deployment, and AI work remain deferred. See [Project Outline](PROJECT_OUTLINE.md#creator-ready-mvp-components) and [Plan](PLAN.md#authorized-multi-slice-work-request-post-review-creator-controls).
