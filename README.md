...
Programmatic editor inspection and manipulation use the versioned [Editor Adapter](EDITOR_ADAPTER.md). In development it is available as `window.scrollSpliceEditor`; the optional authenticated tool dispatcher imports the same adapter directly rather than manipulating the DOM or canvas. Its snapshot includes every persisted element's shared geometry plus type-specific text, shape, balloon, and image properties, plane base colors, and element groups. The adapter can accept host-generated PNG/JPEG/WebP bytes, retain generation provenance in the persistent Asset Library, place the returned stable asset on an explicit plane and logical bounds, and create precisely positioned editable text or shapes through normal undoable editor commands. The upper-right Agent control opens a non-resizing overlay to the left of the inspector and keeps finalized conversation text browser-local by project. The static editor remains completely usable without OpenAI. A separate local-only companion may launch the official Codex App Server for creator-controlled ChatGPT authorization, live GPT-5.5/GPT-5.6 discovery, streamed chat, bounded editor tools, and generated-image intake; credentials never enter browser JavaScript, project files, logs, or git.
...
The implemented creator slice replaces the synthetic drawer with a persistent local Asset Library: the fixed rail exposes **Uploads**, **Speech Balloons**, **Decorations**, and **Splatters**; Uploads contains **All**, **Unsorted**, and creator-named category filters; six original transparent decoration/splatter SVG built-ins and validated PNG/JPEG/WebP imports create real image elements; and creator-named categories and unchanged source `Blob`s persist in IndexedDB. Speech Balloons instead contains ten first-class editable starting types—Standard, Rounded, Thought, Whisper, Shout, Electric, Rough, Wavy, Telepathic, and Double Outline. Every card creates an empty editable balloon body with type, fill, outline, size, and tail controls rather than placing a fixed picture or embedded lettering. Creators add an independent Text element when they want words. Import preflight reads PNG/JPEG/WebP headers, verifies browser decoding, and confirms the declared dimensions without imposing WEBTOON output limits on authoring sources; original bytes and native dimensions stay in the local library. Atomic read-transform-write transactions merge category and import changes made from concurrent tabs, and the tab completing an update refreshes its categories and sources from the saved result. Placed images and editable balloons share the existing canvas/minimap/Layers selection, movement, visibility, deletion, history, save/reopen, and resize behavior. Placement preserves aspect ratio and refuses an extreme source ratio clearly when both axes cannot fit inside the episode while remaining at least 24 logical units. Episode format v4 introduced stable asset IDs, the appearance goal advanced documents to v5, and the current human editor writes bounded format v6 while preserving deterministic v3/v4/v5 opening. WEBTOON dimensions, encoded-file weight, slice count, and package limits apply only during provisional export. Automatic balloon/text coupling, perspective, freeform distortion, multiple linked tails, and creator-saved balloon presets remain deferred.
...
The optional local creation mode can understand a story brief, inspect a safe normalized view of the current project and every persisted canvas property, render a bounded authoritative preview of the complete episode, generate comic images, add them to the asset library, place them on the scroll, and create or revise editable canvas elements through the same document commands used by a human. The isolated companion, disconnected/login UI, streaming transport, five bounded tools, generated-image staging, import, placement, text creation/editing, visual correction, and undo paths are implemented. Katherine completed the official connection and live response proof. On July 21, Codex also cleared the old failed test episode and ran the real Agent UI from a blank canvas: it generated and placed a fresh 800 × 1,280 comic, created separate editable `CAUGHT YOU!` lettering, re-inspected both elements, then revised the lettering's wording, color, size, and position without changing the artwork. A second live proof confirmed that the Agent can use the rendered preview to align three independent dialogue elements with speech balloons that exist only inside flattened generated artwork, including a follow-up correction that moved `SKREEEE!` fully inside a burst balloon.
...
Open the local URL printed by Vite in a desktop Chrome-class browser. Every load or refresh starts with a blank unsaved **Untitled Episode**; saved local projects are retained for explicit **Reopen Current** or **Open Local Project…**, and **Reset Demo** remains available from File. The current local format-v6 release passes 423 Vitest cases across 34 files, strict typecheck, ESLint, production build, all 24 Chromium stories, and 11 companion protocol/security checks. Coverage includes generated-image binary handoff, unrestricted native source dimensions, one-step placement/undo, positioned editable text and shapes, complete type-specific adapter snapshots, bounded visual-preview dispatch as App Server image input, blank startup and the bundled six-image demo, local projects/recovery/portable files, reusable transparent image import, direct placement and appearance, plane organization and stack-direction keys, transforms, ten editable balloon types with direct contour handles, Reader Preview, provisional local export, the larger scrollable minimap, safe reset/reopen behavior, the optional Agent overlay, static-host fallback, explicit login-link state, and finalized-message persistence. The running UI has been checked at representative desktop sizes, including the live generate/place/letter/re-edit and flattened-balloon visual-correction Agent stories above. The latest public-safe views include the July 19 matched [dark](docs/progress/2026-07-19-big-feature-ui-dark.png) and [light](docs/progress/2026-07-19-big-feature-ui-light.png) release screenshots plus the July 20 [real disconnected Agent state](docs/progress/2026-07-20-local-agent-disconnected.png).
...
OpenAI-powered image creation remains optional to the judge path. Katherine completed the bounded local companion proof with an official ChatGPT connection, a live response, and a synthetic generated image placed through ScrollSplice's normal asset/adapter path. The public GitHub Pages editor still requires no account or model access, and the proof does not claim unattended episode creation, a spend meter, cloud storage, or WEBTOON automation.
...
- `src/core/` — framework-independent episode data, geometry, and pure commands
- `src/app/` — Zustand coordination, history, lifecycle, and the public demo fixture
- `src/editor/`, `src/minimap/`, `src/layers/` — the three synchronized editing surfaces
- `src/assets/` and `src/persistence/` — validated local assets, projects, recovery, and portable files
- `src/rendering/` and `src/export/` — shared appearance plus provisional browser-local output
- `src/automation/` and `src/agent/` — the versioned editor adapter and browser-side Agent boundary
- `companion/` — optional localhost-only official Codex App Server integration
- `e2e/` — complete creator-flow browser tests; colocated `*.test.*` files cover unit contracts
