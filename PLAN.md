...
8. **Reliability and handoff — complete:** the July 21 release passes 422 Vitest cases across 34 files, 11 local-companion protocol/security checks, strict typecheck, ESLint, production build, and all 24 Playwright Chromium stories. The editor, Reader Preview, provisional export, matched dark/light release, and disconnected Agent screenshots are indexed, and [Feature Test Sheet](FEATURE_TEST_SHEET.md) records Katherine's review.
...
Completion-pass additions across the historical items are implemented and validated: visible polish and responsive inspector; Finder/canvas/plane/Layers image drop; complete local category/source management; crash recovery; multiple and portable local projects; format-v6 transforms, image masks/crop/frame and episode-edge bleed, flat grouping, populated-plane handling, story-beat movement, ten empty editable balloon types with direct contour handles, provisional rendering, blank clean-profile startup plus the bundled six-image Reset Demo, dual light/dark appearances, compact element grips, the versioned editor adapter, and generated-image intake with exact-bounds placement. Grouped pointer moves preview only the primary member live, then commit all followers together on release. Masks remain clipping boundaries, so first-class panel breakout is still outside this build. The July 21 release passes 422 Vitest cases across 34 files, 11 local-companion checks, typecheck, lint, build, and all 24 Playwright stories. Katherine completed the creator-controlled ChatGPT connection, and Codex completed a clean live Agent generate/place/letter/re-edit proof. Generated-image handoff uses an opaque reference plus an authenticated binary fetch, editor intake retains native source bytes and dimensions while WEBTOON constraints remain export-only, and inspection exposes the persisted type-specific state required for precise follow-up edits.
