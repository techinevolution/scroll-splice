...
8. **Reliability and handoff — complete:** the July 21 release passes 423 Vitest cases across 34 files, 11 local-companion protocol/security checks, strict typecheck, ESLint, production build, and all 24 Playwright Chromium stories. The editor, Reader Preview, provisional export, matched dark/light release, and disconnected Agent screenshots are indexed, and [Feature Test Sheet](FEATURE_TEST_SHEET.md) records Katherine's review.
...
Completion-pass additions across the historical items are implemented and validated: visible polish and responsive inspector; Finder/canvas/plane/Layers image drop; complete local category/source management; crash recovery; multiple and portable local projects; format-v6 transforms, image masks/crop/frame and episode-edge bleed, flat grouping, populated-plane handling, story-beat movement, ten empty editable balloon types with direct contour handles, provisional rendering, blank clean-profile startup plus the bundled six-image Reset Demo, dual light/dark appearances, compact element grips, the versioned editor adapter, generated-image intake with exact-bounds placement, and a bounded authoritative Agent preview. Grouped pointer moves preview only the primary member live, then commit all followers together on release. Masks remain clipping boundaries, so first-class panel breakout is still outside this build. The July 21 release passes 423 Vitest cases across 34 files, 11 local-companion checks, typecheck, lint, build, and all 24 Playwright stories. Katherine completed the creator-controlled ChatGPT connection, and Codex completed clean live Agent generate/place/letter/re-edit and flattened-balloon visual-correction proofs. Generated-image handoff uses an opaque reference plus an authenticated binary fetch, editor intake retains native source bytes and dimensions while WEBTOON constraints remain export-only, structural inspection exposes the persisted type-specific state required for precise follow-up edits, and the read-only preview exposes the rendered composition without a DOM or raw-store escape hatch.
...
The smallest acceptable proof was one creator-authorized request that produced one synthetic image candidate and passed its bytes into the implemented generated-asset import and explicit-plane placement path. Katherine completed the official connection, received a live response, and confirmed that the generated result finished and was placed. The commands record provenance, persist the source, return stable IDs, and use ordinary episode history; undo is covered by adapter/browser validation. The companion exposes progress/cancellation and only five bounded ScrollSplice tools: inspect, preview, apply a validated command, import staged generation, and place an imported asset. It does not expose shell, files, web, connectors, browser control, MCP, private assets, unrestricted agent writes, or WEBTOON credentials. The read-only preview uses the shared renderer and was live-tested by correcting independent dialogue placement against balloons inside flattened generated art.
