...
## 2026-07-21: Keep source-image intake independent from WEBTOON output limits

Decision: Accept browser-decodable PNG, JPEG, and WebP authoring sources at their native byte size and pixel dimensions. Preserve the unchanged source in the local Asset Library. Apply WEBTOON dimensions, output format, encoded-file weight, slice count, and package-total limits only when rendering and validating an export profile. Transfer App Server generations to the browser through a short-lived authenticated binary endpoint addressed by an opaque reference rather than embedding base64 image bytes in Agent event messages.

Reason: Creators need high-resolution and unusually sized working images even though the final publishing package must satisfy a platform profile. The earlier 20 MiB and 40-megapixel intake caps incorrectly treated WEBTOON-oriented output constraints as editor constraints. Inline base64 also caused a completed 992 x 1,586 generated PNG to be rejected as a malformed Agent tool argument before it could enter the Asset Library.
...
Reason: The official App Server is OpenAI's supported local interface for product integrations that need ChatGPT sign-in, streamed turns, model discovery, and host-owned dynamic tools. A static browser cannot safely own reusable OAuth credentials, while copying OpenClaw or Hermes client identities and internal backend calls would create an unsupported and fragile security boundary. The companion preserves the existing local-first editor and lets OAuth authorize only the optional model connection.

Consequences: This narrowly supersedes the Build Week stack rule that prohibited any OAuth/backend work before a separate recorded decision; it does not add a cloud backend, database, OpenAI API key, provider SDK, native desktop wrapper, or WEBTOON integration. The first local connection must begin disconnected and wait for Katherine to click the App Server-provided sign-in URL. Account state remains only in the app-specific Codex state directory or OS credential store; chat remains browser-local per project; episode files, portable projects, recovery, screenshots, logs, and git contain no credentials. Every model write is addressed to one browser session/project/episode, checks an inspected stable ID and document revision, and dispatches through `createEditorAdapter`. Unexpected shell, file-change, MCP, connector, browser-control, or approval events are treated as boundary failures and interrupt the turn. The test build must pin and check the compatible Codex CLI version, explain the local prerequisite, and preserve a no-login reviewer path and truthful demo-video evidence in case judges do not authenticate.
...
Consequences: The normalized creator model is composition group -> layer plane -> element, with stable plane IDs and deterministic group, plane, then local-element order. Plane 1 is lowest inside its group, and increasing plane numbers render above lower numbers. Commit `c5f83c5` implements this as format v3 and directly updates the unsaved fixture because no user project persistence existed at that checkpoint. Background plane 1 may be recolored or hidden but cannot be reordered or deleted. Other planes can be optionally named, safely deleted while empty, and reordered within their group. Dragging ordinary numbered tabs is the intended ordering interaction, using a dedicated grip, stable IDs, and a visible Move Left/Right alternative; overflow arrows scroll the tab strip and never reorder it.

Each group, plane, and element has an independent eye state. Hidden elements remain selectable from Layers but do not render or capture canvas input. Every element has a separate 0–100% opacity control while preserving source alpha. Background planes after the base may hold movable long color regions; creation asks where the region begins and defaults to the current viewport. The implemented **Move to Plane** action is a visible keyboard-accessible control; a future context menu may duplicate it but is not required. Irregular panel masks, breakout/bleed behavior, and additional non-constraining snap targets remain later work.
...
## 2026-07-15: Add a simple persistent asset library with an explicit v3-to-v4 episode upgrade

Decision: After Katherine passed the local history/save/menu checkpoint, make the next bounded creator slice a real local Asset Library. The fixed Add rail contains **Uploads**, **Speech Balloons**, **Decorations**, **Splatters**, and **My Library**. My Library contains creator-named categories rather than adding an unlimited number of buttons to the narrow rail. Built-ins are original code-rendered transparent SVG assets with neutral names; imported source files may be PNG, JPEG, or WebP. A placed asset is a real image element that supports the already implemented selection, movement, visibility, placed-instance deletion, undo/redo, and proportional four-corner resize. This historical slice added no independent text, compound balloon/text behavior, recoloring, tail editing, crop, rotation, flip, opacity, or other asset property controls.

Reason: Katherine requested working uploads, speech balloons, decorations, splatters, creator-made categories, and simple resize-only assets. The old synthetic rectangle drawer cannot represent image alpha or survive reload honestly. A bounded image element and reusable local source library are the smallest coherent foundation; the rail stays finite and the drawer overlays the canvas so opening a real library does not crush the workspace on a smaller supported display.

Consequences: Bump the episode document to format v4 and implement one explicit v3-to-v4 load path for existing local saves; do not add a general migration framework. Keep episode layout in the existing explicit localStorage save slot, storing only stable asset IDs—not image bytes, data URLs, or object URLs. Add a separate IndexedDB-backed `AssetRepository` for creator categories and unchanged source `Blob`s. Imports and category creation use atomic read-transform-write transactions so concurrent tabs merge against the latest snapshot; the successful returned snapshot refreshes categories and runtime sources in the initiating tab. Image import reads PNG/JPEG/WebP header dimensions and enforces the 40-megapixel limit before full decode, then still decodes and confirms the declared dimensions. Imports and category creation persist to the browser library when they succeed and remain reusable across New Episode and Reopen; episode placement still becomes durable only through **File > Save**. Source-library changes are not episode-history entries, while placing, moving, resizing, hiding, and deleting an instance use the existing document history. Placement preserves aspect ratio and clearly refuses an extreme source ratio when both dimensions cannot remain at least 24 logical units inside the episode. A missing source renders an honest placeholder rather than crashing. Source deletion, category rename/delete/reorder, drag-to-place, text editing, shapes/frames, export conversion, cloud sync, and AI-generated assets remain later work. A WebP source must eventually be converted by the exporter because the observed WEBTOON form accepts JPG/JPEG/PNG only.
...
Reason: Katherine's July 15 review confirmed that uploads, alpha, placement, and resizing work well and selected direct drag plus foundational compositing as the next useful creator workflow. Separating placement, opacity, gradient/fade, texture, and blending keeps failures attributable while still sharing one small document-appearance contract. One vertical two-stop model covers both color transitions and transparency fades without introducing an arbitrary gradient editor.

Consequences: Drag data contains only a strictly validated source kind and stable asset ID, uses the existing pan/zoom coordinate seam, creates one normal history entry, and never contains source bytes or object URLs. The appearance slices make one explicit v3/v4-to-v5 compatibility transition rather than a general migration framework. Every element defaults to 100% opacity and Normal blending; old shape opacity is preserved. Zero-opacity elements remain recoverable from Layers and do not capture canvas input. A tiled texture reuses an imported source Blob and gives its coverage freeform bounds, while ordinary images keep proportional resize. Canvas and minimap consume the same appearance data, and compositing needs pixel-level verification in deterministic render order. Operating-system file drop, Layers-panel drops, arbitrary or multi-stop gradient angles, crop/cover, texture-density controls, extra blend modes, masks, advanced transforms, export, cloud services, OAuth, and AI remain outside this goal. This decision approves the contract only; it does not claim implementation, validation, or publication.

Implementation record: The approved contract is implemented and published. Built-in and imported assets support native stable-ID drag placement with click fallback; format v5 carries universal opacity, vertical two-stop color/alpha fills, single/tile image presentation, and exactly Normal, Multiply, Screen, Overlay, and Soft Light through canvas, minimap, bounded history, Save, reload, Reopen, and reset. Tile scale is intentionally automatic with a maximum 160-logical-unit tile edge. The prior PNG/JPEG/WebP import, source-alpha preservation, and ordinary Background-plane photo path remain unchanged; Background plane 1 remains color-only.

Validation record: 255 unit tests across 13 files, strict typecheck, ESLint, production build, focused drag/appearance Chromium coverage, and the full 6-of-6 Playwright Chromium suite pass. The public-safe 1440 × 900 screenshot is visually inspected and indexed. Feature commit `7768daa0617b66c696f769d97dd531f9029272c8` was pushed to `main` July 15. Balloon-outline seam repair and minimap aspect distortion remain deferred polish.

## 2026-07-15: Approve a three-slice creator completion pass without changing format v5

Decision: Implement exactly three ordered creator-facing slices next: (1) optional plane names and same-group plane reordering through a dedicated drag grip plus visible Move Left/Right actions, together with explicit element Bring Forward/Send Backward and Move to Plane; (2) basic independent `TextElement` creation and editing for wording, color, font size, weight, and alignment; and (3) a chrome-free reader preview derived from the current episode document plus unsaved-change confirmation before Reset Demo discards work.

Reason: The published editor already supports the long-scroll canvas, minimap, layers, import, direct placement, appearance, history, and explicit save lifecycle. The largest remaining gaps in a believable manual creator story are correcting composition order without recreating elements, adding readable lettering over the existing balloon artwork, and reviewing the finished scroll without editor chrome. Reset Demo currently deserves the same protection already applied to New Episode and Reopen.

Consequences: Use the existing optional plane `name`, plane `order`, element `layerPlaneId`, element `zIndex`, and complete `TextElement` fields; neither the organization slice nor the text slice needs a format bump. Keep the Layers row list spatially ordered and expose stacking through explicit actions. Background plane 1 stays pinned, drag reordering remains within one fixed group, and Move to Plane may target only ordinary planes while preserving the element's stable identity and source reference. Text remains independent from balloon image assets, so no compound balloon/tail model is introduced. Reader preview is a read-only view of the authoritative document and preserves selection, active plane/group, zoom, viewport, history, and dirty state. A confirmed Reset Demo loads the fixture as unsaved while keeping the last explicit save available to Reopen; cancel changes nothing. Every document change enters bounded undo/redo and explicit Save/reload/Reopen behavior.

This approval does not include WEBTOON export, an OpenAI runtime, AI generation, OAuth, operating-system/Finder file drop, autosave or crash recovery, source deletion, masks, crop/cover, rotation, flipping, perspective, editable balloon tails, or a compound balloon/text entity. It authorizes implementation of the three recorded slices but does not pre-claim their test counts, completion, publication, or deployment.

Implementation record: All three slices are implemented locally without changing format v5. Ordinary planes support optional names, same-group grip or Move Left/Right ordering, and pinned-base protection; selected elements support explicit local Bring Forward/Send Backward and Move to Plane. Independent text supports wording, color, size, weight, and alignment through the shared canvas, minimap, Layers, appearance, history, and persistence paths. **View > Reader Preview** renders the authoritative document without editor chrome and restores editor context on exit. Dirty Reset Demo can be canceled as a complete no-op or confirmed as an unsaved fixture reset while leaving the explicit saved slot recoverable through Reopen.

Validation record: 270 unit tests across 13 files, strict typecheck, ESLint, the production build, and all 7 Playwright Chromium stories pass. The creator story covers plane organization, a balloon plus independent text, stack correction, Move to Plane, Reader Preview, Save/reload/Reopen, reset cancel/confirm, and saved-slot recovery. The July 16 public-safe 1440 × 900 screenshot and Reader Preview were visually inspected. The production build emits Vite's known non-blocking size advisory for a 637.55 kB minified JavaScript chunk. Feature commit `cb1f28443f7b1045d139879a2bba7b03edf25856` was published to `main` July 16, and local/remote equality was verified immediately after the push. Deployment and Devpost submission remain separate evidence gates.

## 2026-07-16: Replace the three-slice stop with the full locally feasible human-editor goal

Decision: Katherine supersedes the historical three-slice stopping point and authorizes continued implementation of the documented local human-editor backlog before the feature test sheet is prepared. Work in this order: visible workspace polish; operating-system drop and complete local asset management; crash recovery plus portable/multiple local projects; transforms, snapping, lock, grouping, and section organization; rectangular/polygon panel frames with crop/cover and intentional breakout; compound balloon/text editing; then a local tall-master and deterministic-slice proof. Preserve one coherent, validated checkpoint per slice rather than requesting another product choice after every three slices.

Reason: The published format-v5 editor already proves the defining workspace and a credible creator story. Katherine wants the remaining local workflow completed as far as practical so her next review can use one comprehensive feature sheet instead of repeatedly stopping to choose from a known backlog.

Consequences: Format v5 remains the confirmed baseline until a schema-dependent feature lands. That feature may create one bounded format v6 adding only common rotation/flip/lock fields, explicit crop/cover appearance, flat rectangular or normalized-polygon panel-frame records, and flat non-nested composition records that associate stable member IDs for grouping and balloon-plus-text or panel-plus-image behavior. Supported v3/v4/v5 documents receive deterministic v6 defaults; unknown formats fail safely. Do not create recursive groups, a generic scene graph, a plugin schema, destructive source edits, or a broad migration framework.

The active goal may extend the existing local `ProjectRepository` and `AssetRepository`, but it does not authorize a backend, database service, cloud sync, OAuth, OpenAI runtime dependency, external connectors, native desktop packaging, private-asset transfer, or WEBTOON automation. A local renderer may use the current `form-observed` WEBTOON profile only when its output is visibly labeled **provisional, not upload-verified, and not guaranteed WEBTOON-ready**. Manual upload discovery remains required before any verified-compatibility claim, and publishing stays manual. Public deployment and Devpost evidence remain their dated submission gates.

## 2026-07-16: Complete the local editor with bounded format v6 and provisional output

Decision: Implement the broad local-human-editor goal as one bounded format-v6 extension plus application-edge adapters. V6 adds common rotation/flip/lock/overflow fields, explicit image crop/mask/frame data, flat non-nested element groups, and one atomic editable speech-balloon element. Multiple local projects, debounced recovery, portable project files, complete asset-source/category lifecycle actions, and local rendering remain outside the episode model.
...
## 2026-07-21: Pair structural inspection with a bounded visual preview

Decision: Add one read-only `preview_editor` operation to the authenticated Agent tool boundary. It renders the current authoritative episode through the shared tall-master renderer, bounds the result to 400 by 2,000 pixels, and supplies the JPEG to the official App Server as `inputImage` together with the post-render structural snapshot. The Agent must preview after placing generated art, preview again after adding lettering, and correct visible placement before claiming completion.

Reason: Stable IDs, exact bounds, and editable properties are sufficient for manipulating document records, but they cannot describe visual features baked into a flattened generated image. In the live gardener-and-goblin proof, blank speech balloons existed only as pixels, so the Agent needed to see the rendered composition to place independent dialogue reliably.

Consequences: Inspection remains authoritative for IDs, revisions, and coordinates; visual inference never becomes hidden document state. The preview is bounded and read-only, returns no raw source bytes or filesystem path, and does not expose the DOM, Konva, Zustand, credentials, or a general screenshot capability. The authenticated registry now has exactly five tools. A live correction loop moved only `SKREEEE!` until the full word had visible padding inside its burst balloon while preserving the artwork and other dialogue.
