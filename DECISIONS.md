...
Consequences: The composition-groups and visibility checkpoint and the numbered layer-plane and editable episode-backdrop foundation were completed July 13. Asset properties and opacity is the next recommended slice, but it remains unapproved; background-region, layer-management, Add-rail, import, resize, persistence, export, deployment, and AI work remain separately gated. Stop starting new product features after July 18. Public access and submission evidence remain mandatory but must not displace the earlier product window.
...
Consequences: The implementation reuses format-v3 `name` and `logicalHeight`; no format bump was needed. `setEpisodeName` accepts a trimmed, nonblank name up to 60 characters, while the inline header editor commits on Enter or blur and cancels on Escape. `extendEpisodeHeight(amount)` is extend-only, and **Add scroll space** dispatches one centralized `1280`-unit default without moving content or the viewport. Background plane 1 derives its coverage from episode height, while the minimap independently refits the full episode. `deleteEmptyLayerPlane` deletes only an ordinary plane with no elements—hidden elements still count—protects the base, rejects a group's final plane, compacts that group's order, and leaves the nearest remaining plane active. The empty-state's paired paperclip **Attach asset** control is deliberately disabled future-manual-placement signaling, not asset functionality. The current editor remains fixed-fit. Adjustable zoom stays transient future editor state and requires separate approval before it supersedes that historical decision; opacity follows it in the proposed sequence. The checkpoint is complete locally, while remote publication remains separately gated.
...
Consequences: Title editing activates from the title text without permanent edit chrome. Element-row trash actions remove only placed instances, and the disabled paperclip remains available in empty and populated ordinary planes without claiming real import. `setBaseColor` owns both Layers and canvas controls. Precise height changes clamp at the lowest bound of every visible or hidden element, while Background plane 1 continues to derive from episode height. Solid color regions are ordinary full-width Background elements with logical start, height, color, selection, visibility, movement, and deletion. Checkpoint C adds transient Fit Width/50–200% zoom and two-dimensional viewport geometry. Each checkpoint requires its own tests and visual QA. This decision authorizes implementation but does not authorize a `main` push. Real uploads, asset persistence, undo, gradients, opacity, production export, deployment, OAuth, and AI remain deferred.
