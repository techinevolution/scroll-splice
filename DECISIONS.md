...
## 2026-07-13: Propose Episode Setup and Expandable Scroll before adjustable zoom and opacity

Decision: Record the recommended remaining creator-facing order as episode structure, then adjustable zoom, then asset opacity, pending Katherine's approval of each implementation slice. The episode-structure checkpoint makes the existing episode name editable, lets the creator extend the episode from a `+` at the bottom of the story canvas, keeps the minimap fitted to the complete episode, and permits safe deletion of an empty ordinary layer plane. The following zoom checkpoint replaces fixed Fit-only viewing with **Fit Width** and 50–200% zoom while preserving access to the complete canvas.
...
Consequences: Reuse the existing format-v3 `name` and `logicalHeight` fields; do not bump the format merely to expose them. `setEpisodeName` accepts a trimmed, nonblank name up to 60 characters. `extendEpisodeHeight(amount)` is extend-only, and the first bottom `+` uses one centralized default increment of `1280` logical units. Background plane 1 derives its coverage from the episode height, while the minimap independently refits the full episode. `deleteEmptyLayerPlane` may delete only an ordinary plane with no elements—hidden elements still count—and must protect the base, reject deletion of a group's final plane, and leave the nearest remaining plane active. Adjustable zoom is transient editor state using a zoom factor and two-dimensional viewport; it preserves the logical center where possible and clamps both axes. The historical fixed-fit decision remains accurate for the currently implemented checkpoint and is superseded only when the separately approved zoom slice lands. This roadmap decision and its documentation do not authorize feature implementation.
