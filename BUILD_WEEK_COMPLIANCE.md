...
- [x] An isolated Playwright Chromium context can complete the expanded defining story, including title validation, plane/element controls, synthetic placement, episode-height changes, freely movable and independently resizable Background regions, magnet snap/bypass, live status `x/y/w/h` and minimap previews, bounded zoom, minimap agreement, and reset. The paperclip path is accurately limited to synthetic demo rectangles and is not claimed as real image import.
...
- [x] The optional local history/save/menu slice passes 154 unit tests, strict typecheck, ESLint, production build, two isolated Chromium tests, and visual inspection at 1440 × 900, 1280 × 720, and 1024 × 768. This creator-workflow addition is not a new contest requirement; Katherine's hands-on review and any push remain pending.
...
| Local history/save/menu slice | 100-checkpoint document history; create/delete/move/resize/title/height/layer-change undo; one explicit versioned browser save; reload auto-open; confirmed Reopen and New Episode; exact File/Edit menus and shortcuts | 154 unit tests, strict typecheck, ESLint, production build, two isolated Playwright Chromium tests, and supported-size visual inspection pass locally; Katherine review pending; not pushed |
