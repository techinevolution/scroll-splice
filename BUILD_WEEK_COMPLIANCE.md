...
- [x] Katherine performed the first creator-controlled ChatGPT login, received a live reply, and confirmed a synthetic generated image finished and was placed. Adapter/browser tests cover import/place/undo; no auth URL, account identity, or credential was recorded.
...
- [x] The bundled **The Light We Planted** Reset Demo combines the existing code-defined fixture elements with six original OpenAI-generated story panels; it contains no private comic art or third-party reference images. A clean browser now starts from a blank **Untitled Episode** instead.
- [x] Typecheck, lint, all 417 unit tests, production build, and the complete editor Playwright smoke test pass from a clean detached checkout of final commit `7abf82a` on July 21.
...
- [x] The historical persistent Asset Library stack passed 214 unit tests across 11 files, strict typecheck, ESLint, production build, four Chromium stories, and supported-size visual inspection. At that checkpoint header preflight enforced a 40-megapixel intake limit; Katherine's July 21 clarification supersedes that product rule so native source dimensions remain available for authoring and WEBTOON limits apply only during export. Its upload story proves alpha preservation against sampled underlying canvas pixels and carries uploaded-image resize through undo/redo, Save, reload, and Reopen. Feature commit `fdd4ead37e7071bc7c69c9c4d8b49c557ddd95d7` was published and verified on local and remote `main` July 15.
- [x] The July 15 review correction proves application-menu hit testing above the open Asset Library, compact **Decor** label containment, and active-category toggle-close in Chromium; its public-safe screenshot contains no uploaded user art. Commit `3ec9bd095fab5ba2fb19f9d97cfeb79fcdbceae5` was published and verified on local and remote `main`.
- [x] The published direct-placement and foundational-appearance goal gives original built-ins and imported assets native drag-to-canvas placement with the existing click fallback. Existing PNG/JPEG/WebP import, source alpha, and photo placement on Background plane 2 or later remain confirmed; Background plane 1 remains the color-only base.
- [x] The published format-v5 appearance model provides universal element opacity with one history entry per gesture, vertical two-stop color-and-alpha gradients, single/tile image presentation with a fixed automatic scale capped at a 160-logical-unit tile edge, and exactly Normal, Multiply, Screen, Overlay, and Soft Light blend modes. Canvas, minimap, persistence, and history all use the same saved appearance state.
- [x] The direct-placement and appearance goal passes 255 unit tests across 13 files, strict typecheck, ESLint, production build, targeted drag/appearance Chromium coverage, and the repaired full Playwright Chromium suite at 6/6. Its 1440 × 900 public-safe screenshot was visually inspected.
- [x] Feature commit `7768daa0617b66c696f769d97dd531f9029272c8` was pushed to `main` July 15; balloon-outline seam cleanup and minimap aspect distortion remain deferred polish.
...
- [x] Final README contains repository URL, access URL, accurate current status, verified commands, sample-data explanation, Build Week boundary, and Codex/GPT-5.6 collaboration evidence.
...
| Menu and Asset Library polish screenshot | `docs/progress/2026-07-15-menu-and-asset-rail-polish.png`, 1440 × 900 PNG, SHA-256 `aa160a27ef595f06b8f93261c595851b7f8fca0a70a3224b114d478ba7e1d3d7`; File menu above the open Decor drawer, compact label, original Radiance accent, and synthetic fixture only | Published and verified in `3ec9bd095fab5ba2fb19f9d97cfeb79fcdbceae5` |
| Direct-placement and format-v5 appearance goal | Native drag for original built-ins and imported PNG/JPEG/WebP assets with click fallback; source alpha; Background plane 2+ photos; universal opacity; vertical two-stop color-and-alpha gradients; single/tile presentation at a fixed automatic scale capped at a 160-logical-unit tile edge; Normal, Multiply, Screen, Overlay, and Soft Light; shared canvas/minimap/persistence/history behavior | Published and verified in `7768daa0617b66c696f769d97dd531f9029272c8` |
| Direct-placement and appearance validation | 255 unit tests across 13 files, strict typecheck, ESLint, production build, targeted drag/appearance Chromium coverage, and repaired full Playwright Chromium suite 6/6 | Complete July 15 before publication |
...
| Optional local Agent companion | Official `codex-cli 0.144.5` App Server in isolated local state; localhost/capability/origin protections; disconnected/static fallback; GPT-5.5/GPT-5.6 model filtering; persistent overlay; bounded revision/context-checked editor tools; generated-image staging/import/place/undo | Disconnected and synthetic protocol paths validated July 20 with 411 Vitest cases across 34 files, 11 companion checks, typecheck, lint, production build, and all 19 Playwright Chromium stories; Katherine then completed the official connection and live reply, and the July 21 clean Agent test generated and placed native art, added editable lettering, inspected it, and revised the existing text without altering the art |
| July 21 final release | Blank startup; explicit Reset Demo; ten empty editable balloon types with independent Text; current project, export, appearance, adapter, and Agent behavior | 417 Vitest cases across 34 files, 11 local-companion checks, strict typecheck, ESLint, production build, and all 24 Playwright Chromium stories pass; Vite retains only its non-blocking bundle-size advisory |
