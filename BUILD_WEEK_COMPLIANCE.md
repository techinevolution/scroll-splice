...
- [x] Katherine's July 14 reviews confirmed the A/B/C controls, exposed the first polish defects, and then superseded the fixed-width Background correction with freely editable Background-region geometry plus live status/minimap previews. The latest 123-test implementation, full local validation, expanded browser walkthrough, and supported-size visual inspection pass; Katherine's retest remains open.
