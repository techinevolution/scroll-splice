...
The normalized inspection snapshot exposes all persisted canvas state needed for a precise follow-up edit: shared geometry and ordering, plane base colors, group membership, text content, shape styling, balloon body/tail properties, and image presentation/frame/mask/crop values. It intentionally excludes React/Konva objects, transient open-panel state, raw image bytes, object URLs, credentials, and filesystem paths. The model can therefore reason about and change the authoritative document without receiving a DOM or raw store escape hatch.
