...
2. Upload validates PNG/JPEG/WebP signature and optional creator-category identity, parses declared dimensions from PNG IHDR, JPEG SOF, or WebP VP8/VP8L/VP8X headers, then asks the browser decoder to verify the source and confirm the header dimensions before the unchanged source is saved. Authoring intake does not apply WEBTOON output dimensions or encoded-byte limits to source images.
...
The adapter now also owns a narrow asynchronous generated-image intake seam. An authorized host may provide a PNG/JPEG/WebP `Blob`, base64 data URL, or raw base64 payload plus non-secret provider/model/prompt/timestamp metadata. The adapter applies the same header, media-type, and browser-decode validation as human imports without applying WEBTOON output limits, persists the unchanged Blob and provenance through `AssetRepository`, and returns its stable asset ID. The local companion sends only a short-lived opaque generation reference through the Agent event stream; the browser retrieves the corresponding binary response from the authenticated same-origin companion endpoint and supplies that Blob to the adapter. Source bytes and filesystem paths never enter the model-visible tool arguments. A separate placement command creates one imported-image element on an explicit ordinary plane at requested logical bounds through one normal history checkpoint. Exact positioned text and shape commands create editable lettering and panels through the same core commands. Undo removes any placed or created instance while retaining a reusable generated source.

The normalized inspection snapshot exposes all persisted canvas state needed for a precise follow-up edit: shared geometry and ordering, plane base colors, group membership, text content, shape styling, balloon body/tail properties, and image presentation/frame/mask/crop values. A separate read-only preview tool renders that same document and its resolved assets through the shared tall-master renderer, bounds the result to 400 by 2,000 pixels, and returns it as an App Server `inputImage`. Structural inspection remains authoritative for IDs and coordinates; the preview supplies visual semantics that do not exist in the document when speech balloons, panels, or other details are pixels inside one flattened generated image. It intentionally excludes React/Konva objects, transient open-panel state, raw source bytes, object URLs, credentials, and filesystem paths. The model can therefore reason about and change the authoritative document without receiving a DOM or raw store escape hatch.
...
- `EditorToolRegistry`: exactly five model-visible tools inspect the editor, render a bounded read-only visual preview, apply a validated reversible command, import the latest generated source, or place a generated source on explicit logical bounds.
...
The generated-image intake, exact-bounds placement, positioned editable text/shape creation, complete persisted-property inspection, bounded authoritative visual preview, official App Server authorization path, bounded chat/run UI, creator-visible progress/cancellation, and returned-byte handoff are implemented. The disconnected and synthetic protocol paths are validated without exposing credentials or account identity. Katherine completed the creator-controlled connection and live response proof. On July 21, a clean real-Agent run generated and placed a fresh 800 × 1,280 comic, added separate editable lettering, verified the image and exact text through inspection, and then revised the lettering's wording, color, size, and position while preserving the artwork. A later three-panel gardener-and-goblin proof used `preview_editor` to see blank balloons inside flattened art, place three independent text elements, and visually correct the final `SKREEEE!` lettering until it had clear padding inside its burst balloon. The static human editor remains the valid judge path without OpenAI.
...
The July 21 release passes 423 Vitest cases across 34 files, 11 Node local-companion protocol/security checks, strict TypeScript, ESLint, the production build, and all 24 Playwright Chromium stories. Its added coverage verifies all ten stable editable type IDs, complete type-specific adapter snapshots, exact positioned text and shape creation, bounded visual-preview dispatch as App Server image input, shared geometry, empty new bodies, direct normalized contour changes, preservation and export parity of preset-specific treatments after contour editing, persistence, hidden compatibility resolution for retired fixed balloons, blank startup, explicit demo loading, generated-image binary retrieval, native-dimension source intake, and the complete editor/export lifecycle. The production build retains Vite's non-blocking over-500 kB advisory.
