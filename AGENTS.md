...
- Treat the existing episode `name` and `logicalHeight` as document data. Title edits trim input, reject a blank name, and enforce the 60-character editor limit through `setEpisodeName`. Height changes are extend-only and go through `extendEpisodeHeight`; do not move existing elements or add a second stored height for the base plane.
- Background plane 1 always derives its full-scroll coverage from `episode.logicalHeight`. The bottom **Add scroll space** control is editor chrome, never an episode element, layer, minimap item, or export item. Keep `DEFAULT_EPISODE_HEIGHT_INCREMENT = 1280` as the single source for its increment.
...
- Treat the disabled paperclip **Attach asset** button beside the empty-plane delete action as a future manual-placement placeholder only. It must remain disabled and must not be reported as import, attachment, placement, or drag-and-drop functionality until a later approved asset slice implements that path.
...
- Katherine's one-time authorization to publish `cd08bb4`, `c5f83c5`, `2478e87`, and their feedback-planning documentation completed in `d8f8008`. Katherine subsequently approved Episode Setup and Expandable Scroll for local implementation and commit, but no new `main` push authorization was recorded. Do not infer authorization for that push, adjustable zoom, opacity, deployment, OpenAI integration, OAuth, external services, or other later work.
