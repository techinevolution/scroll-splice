...
- Keep built-in assets original, local, transparent, neutrally named, and source-stable. The local build supports click/drag placement, selection, movement, eye/trash, undo/redo, minimap/Layers synchronization, proportional four-corner resize, universal opacity, the five recorded blend modes, vertical two-stop Background fills, and Stretch/Cover/Tile image presentation. General selected-image controls implement crop focus/zoom, rotation, and flip; the atomic editable balloon implements fitted text and tail controls. Static built-in artwork recoloring, coupling a loose balloon image to separate text, perspective, freeform distortion, and other asset-specific editing remain deferred. Tile scale is automatic and capped at a 160-logical-unit tile edge.
