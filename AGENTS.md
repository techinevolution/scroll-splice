...
- Model each solid Background color region as an ordinary full-width element with logical vertical bounds and color in an ordinary Background plane. It is structurally `x = 0`, 800 units wide, and vertically movable only; the magnet toggle or temporary bypass must not release that invariant. It participates in normal selection, visibility, movement, deletion, minimap rendering, and safe-height calculations. Do not add gradients, blend modes, or imported backgrounds without a later approved slice.
- Treat ordinary alignment snapping as default-on editor state with a visible magnet toggle. Checkpoint D implements only one real target: snap an ordinary element's horizontal center to the episode centerline when it is within 8 CSS pixels at the current zoom, show a temporary vertical center guide, and bypass with magnet off or Alt/Option held during the drag. Defer edge and nearby-element targets. Snapping must never silently resize or crop content.
- Keep checkpoint D's resize deliberately bounded: only a selected unlocked ordinary element gets four proportional corner handles. Route the committed bounds through pure `resizeElement`, preserve aspect ratio, clamp within the episode, enforce the 24-logical-unit minimum, and scale Text font size with its bounds. Never expose those handles on a full-width Background color region, and do not add rotation, flipping, side handles, crop, or freeform distortion without a later approved slice. This uses existing format-v3 bounds.
...
- Regression-test the implemented title validation and reset, empty-plane deletion guards and fallback selection, bottom-footer synthetic placement in empty and populated planes, coarse and precise height changes, automatic base coverage, minimap refitting, and repeated bottom-`+` use. Keep focused coverage for anchored click-to-edit title activation, synchronized Layers/canvas base controls, placed-element deletion, safe drag/keyboard growth and shrink clamping, the live and committed Background color-region `x = 0` invariant, magnet toggle and Alt/Option bypass, profile-guide alignment, four-corner proportional resize and minimap synchronization, Fit Width, 50% and 200% bounds, center preservation, two-axis clamping, element movement at non-default zoom, and accurate minimap viewport geometry at the same representative sizes.
...
- The local corrective checkpoint is evidenced separately by 120 unit tests, strict typecheck, ESLint, production build, one isolated expanded Chromium walkthrough, and visual inspection at the same three supported desktop sizes.
- Katherine's July 14 A/B/C human test confirmed that placed-element deletion and adding a synthetic asset from the bottom of a populated plane work. Her next review failed the title-anchor, guide, live Background-region, magnet, and corner-resize expectations. The corresponding corrective implementation now passes its local automated and visual validation, but do not report it as human-approved or pushed until those separate gates actually pass.
