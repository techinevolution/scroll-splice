...
- Treat the existing episode `name` and `logicalHeight` as document data. A planned title edit must trim input, reject a blank name, and enforce the documented 60-character editor limit. A planned height change is extend-only and must go through `extendEpisodeHeight`; do not move existing elements or add a second stored height for the base plane.
- Background plane 1 always derives its full-scroll coverage from `episode.logicalHeight`. The planned bottom `+` is editor chrome, never an episode element, layer, minimap item, or export item. Keep the first `1280`-unit extension increment in one named constant.
...
- Delete a layer plane only through the planned pure `deleteEmptyLayerPlane` command. It may delete an ordinary plane only when no element references it; hidden elements still count as contents, Background plane 1 is protected, and a group's final plane cannot be deleted. The store must activate the nearest remaining plane afterward.
...
- On July 13, Katherine explicitly authorized the immediate push of pending commits `cd08bb4`, `c5f83c5`, and `2478e87` together with the current feedback-planning documentation commit directly to `main`. This is authorization for that specific push only. After it completes, no standing authorization remains for implementing or pushing the planned episode-structure, adjustable-zoom, or opacity slices, nor for deployment, OpenAI integration, OAuth, external services, or other later work. Obtain a new explicit instruction before those actions.
