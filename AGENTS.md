...
- Treat the existing episode `name` and `logicalHeight` as document data. Title edits trim input, reject a blank name, and enforce the 60-character editor limit through `setEpisodeName`. The title text itself activates editing; do not keep a permanent pencil, input border, or cursor before activation.
...
- Keep the disabled paperclip **Attach asset** affordance below every ordinary active plane's element list, including populated planes. Pair it with empty-plane deletion when applicable. It must remain disabled and must not be reported as import, attachment, placement, or drag-and-drop functionality until a later approved asset slice implements that path.
...
- Model each approved solid Background color region as an ordinary full-width element with logical vertical bounds and color in an ordinary Background plane. It participates in normal selection, visibility, movement, deletion, minimap rendering, and safe-height calculations. Do not add gradients, blend modes, imported backgrounds, or special renderer-only state in that checkpoint.
...
- Keep the current checkpoint fixed-fit until authorized Canvas Zoom and 2D Viewport checkpoint C is implemented and validated. Its zoom model uses transient `zoomFactor`, `viewportX`, and `viewportY`, replaces the passive fit readout with **Fit Width** plus 50–200%, calculates scroll progress over the navigable range, preserves the logical center where possible, clamps both axes, and gives the minimap an accurate two-dimensional viewport box. Do not put zoom in the episode document.
...
- Regression-test the implemented title validation and reset, empty-plane deletion guards and fallback selection, disabled attachment placeholder, coarse height extension, automatic base coverage, minimap refitting, and repeated bottom-`+` use. For authorized checkpoints A/B/C, add focused coverage for click-to-edit title activation, synchronized Layers/canvas base controls, placed-element deletion, the paperclip in populated planes, safe drag growth/shrink clamping, solid Background color regions, Fit Width, 50% and 200% bounds, center preservation, two-axis clamping, and accurate minimap viewport geometry at the same representative sizes.
- The local Episode Setup and Expandable Scroll checkpoint is evidenced by 63 unit tests, strict typecheck, ESLint, production build, one isolated Chromium walkthrough, and visual inspection at 1440 × 900, 1280 × 720, and 1024 × 768.
...
- Katherine's one-time authorization to publish `cd08bb4`, `c5f83c5`, `2478e87`, and their feedback-planning documentation completed in `d8f8008`. She subsequently approved Episode Setup and Expandable Scroll for local implementation and commit, then authorized three local checkpoints in order: Direct Creator Controls, Safe Precise Height with solid Background Color Regions, and Canvas Zoom/2D. No new `main` push authorization was recorded. Do not infer authorization to push these checkpoints or to implement opacity, real imports, persistence, undo, export, deployment, OpenAI integration, OAuth, or external services.
