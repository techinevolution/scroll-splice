...
1. Call `inspect()`.
2. Record `snapshot.episode.id` and `snapshot.editor.currentRevision`.
3. Find the target by its returned stable ID. Never invent or infer an ID from visible text alone.
4. Issue one `execute(command)`. Network-dispatched mutations must also supply the recorded episode ID and expected revision.
5. Check `result.ok`, `result.changed`, and the returned snapshot.
6. Use `undo` if the result is valid but not what the creator wanted.
